...
- **Scan lines** — a fixed full-viewport `<canvas>` element (`z-index: 9999`, `pointer-events: none`) that draws 2px dark / 2px transparent horizontal stripes scrolling downward at 8 CSS px/second, simulating the rolling refresh of a CRT phosphor display. Implemented in canvas rather than CSS (`repeating-linear-gradient`) because iOS Safari at 3× DPR renders gradient periods at non-integer device-pixel positions, producing Moiré banding. The canvas is allocated at native resolution (`width = innerWidth × devicePixelRatio`) and all stripe positions are computed as `Math.round(offset × dpr)`, ensuring integer device-pixel alignment on every frame. Animation runs via `requestAnimationFrame` with delta-time accumulation — no CSS keyframe loop, no compositing seam at the cycle boundary.
